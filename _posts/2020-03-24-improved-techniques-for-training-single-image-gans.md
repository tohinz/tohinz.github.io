...
While training the GAN on several stages of different resolutions seems to be a good way of training a GAN on a single image, the question is how many stages we need to train until we can generate good images. The original SinGAN model, on average, needs to train on 8-10 individual stages. This obviously increases both the number of parameters in the model and the training time.
...
\begin{equation}
    $x_n = x_N\times r^{N-n},
\end{equation}
where $r=0.75$ is used as default value for the scaling factor and $x_N$ is the image resolution used for the final generator. For example, for an image $x_N$ of resolution 188x250 px and a training of ten stages this means an image pyramid of ($26\times34, 32\times42, 40\times53, 49\times66, 62\times82, 77\times102, 96\times128, 121\times160, 151\times200, 188\times250$).

We can downsample the images more aggressively to train the model on fewer stages, e.g. using $r=0.55$ leads to an image pyramid with six stages of ($25\times34, 38\times50, 57\times75, 84\times112,126\times167,188\times250$). However, if we do this with the original rescaling approach the generated images lose much of their global coherence.
...
We now show some of our results.

Unconditional image generation:

<figure><center><img src="/images/blog/ConSinGAN/examples/unconditional-generation.jpg" alt="Unconditional Image Generation" width="50%"/><figcaption>Fig. 7: Examples of our model for unconditional image generation.</figcaption></center></figure>

Image harmonization:

<figure><center><img src="/images/blog/ConSinGAN/examples/harmonization.jpg" alt="Image Harmonization" width="50%"/><figcaption>Fig. 8: Examples of our model for image harmonization.</figcaption></center></figure>

These are just some of our results. For more results and details please refer to our [paper and supplementary material](https://www.google.com){:target="_blank"} or try it for yourself with the [code we provide](https://github.com/tohinz/ConSinGAN){:target="_blank"}.
